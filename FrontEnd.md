...
Math.round()//四舍五入，注意-3.5 结果为-3- [网络请求](#网络请求)
  - [HTTP](#http)
    - [http状态码](#http状态码)
      - [**`1XX`：信息状态码**](#1xx信息状态码)
      - [**`2XX`：成功状态码**](#2xx成功状态码)
...
- [HTML](#html)
  - [通用](#通用)
    ...
    - [通用类名](#通用类名)
    - [标签语义化](#标签语义化)
    - [图标字体 icontfont](#图标字体-icontfont)
  - [标签](#标签)
    - [meta](#meta)
    - [link](#link)
    - [列表`ul、ol、il`](#列表ulolil)
    - [表格table](#表格table)
    - [form](#form)
    ...
    - [`input`](#input)
      - [清除输入框历史记录](#清除输入框历史记录)
      - [使input文本框不可编辑的3种方法](#使input文本框不可编辑的3种方法)
    - [弹性模型 `display：flex`](#弹性模型-displayflex)
  - [H5](#h5)
    - [变形transform（旋转、缩放、移动、倾斜）](#变形transform旋转缩放移动倾斜)
    - [canvas画布](#canvas画布)
    - [SVG](#svg)
    - [滤镜：filter（过渡效果）](#滤镜filter过渡效果)
  - [CSS](#css)
    - [**CSS优先级**](#css优先级)
    - [**图片背景background**](#图片背景background)
    - [**边框:border**](#边框border)
    - [**内边距:padding**](#内边距padding)
    - [外边距margin](#外边距margin)
    - [浮动float](#浮动float)
      - [**清除浮动方式**](#清除浮动方式)
    - [元素的显示模式转换display](#元素的显示模式转换display)
    - [lang属性](#lang属性)
    - [font属性](#font属性)
    - [文本属性text](#文本属性text)
    - [计量单位](#计量单位)
  ...
  - [定位](#定位)
    ...
    - [网页布局如何规避脱标流](#网页布局如何规避脱标流)
    - [CSS元素的可见性(把元素隐藏起来)](#css元素的可见性把元素隐藏起来)
  - [案例](#案例)
    - [logo内容移除](#logo内容移除)
    - [盒子阴影](#盒子阴影)
    - [精灵图](#精灵图)
    - [滑动门](#滑动门)
  - [问题](#问题)
    - [**图片自动为父元素大小**](#图片自动为父元素大小)
    - [**为什么第一个子元素设置margin-top父元素会跟着移动**](#为什么第一个子元素设置margin-top父元素会跟着移动)
    - [文字环绕图片](#文字环绕图片)
    - [消除父代与子代间距](#消除父代与子代间距)
    - [**DIV的高度随着内容的增加而增加**](#div的高度随着内容的增加而增加)
    - [页面或者按钮为什么会抖一下？](#页面或者按钮为什么会抖一下)
    - [CSS解决border影响元素宽高的问题（box-sizing属性）](#css解决border影响元素宽高的问题box-sizing属性)
    - [a标签内容超出换行](#a标签内容超出换行)
    - [a标签内容超出以后显示省略点](#a标签内容超出以后显示省略点)
    - [把文字下滑线与文字距离宽一点](#把文字下滑线与文字距离宽一点)
    - [内容框的输入法不能转换，固定输入法：`ime-mode:disabled； `](#内容框的输入法不能转换固定输入法ime-modedisabled-)
    - [table隐藏tr/td时，显示错乱问题](#table隐藏trtd时显示错乱问题)
    - [取消表单项聚焦时产生的黄色边框](#取消表单项聚焦时产生的黄色边框)
    - [CSS隐藏页面元素](#css隐藏页面元素)
  - [meta标签详解](#meta标签详解)
    - [HTML5前](#html5前)
      - [name](#name)
...
- [JS](#js)
  - [JS理念](#js理念)
    - [JS是什么？](#js是什么)
      ...
      - [js输出方式](#js输出方式)
      - [JS特点：](#js特点)
      - [Document 和 body 之间的区别](#document-和-body-之间的区别)
    - [事件循环`Event Loop`与JS循环机制](#事件循环event-loop与js循环机制)
      - [浏览器事件循环](#浏览器事件循环)
      - [浏览器与Node的事件循环(Event Loop)的区别](#浏览器与node的事件循环event-loop的区别)
    - [抽象语法树`AST`](#抽象语法树ast)
    - [一次js请求的缓存处理](#一次js请求的缓存处理)
  - [DOM](#dom)
    - [script属性](#script属性)
    - [DOM节点属性](#dom节点属性)
    - [DOM方法](#dom方法)
    - [DOM属性](#dom属性)
    - [DOM访问](#dom访问)
    - [DOM修改（操作样式）](#dom修改操作样式)
    - [DOM事件](#dom事件)
    - [DOM实例](#dom实例)
    - [DOM event 对象](#dom-event-对象)
  - [BOM](#bom)
    - [**onload && onunload**](#onload--onunload)
    - [mouseover、mouseente](#mouseovermouseente)
    - [定时器](#定时器)
    - [location 对象](#location-对象)
    - [**history对象**](#history对象)
    - [popstate事件](#popstate事件)
  ...
  - [方法](#方法)
    ...
    - [new](#new)
  - [数据类型](#数据类型)
    - [`undefine`与`null`](#undefine与null)
    - [数据类型转换](#数据类型转换)
    - [**数字Number**](#数字number)
    - [**字符串String**](#字符串string)
    - [计算精度问题](#计算精度问题)
    - [数组](#数组)
    - [可变参 arguments](#可变参-arguments)
    - [JSON](#json)
  - [对象](#对象)
    - [对象操作](#对象操作)
    - [this](#this)
    ...
    - [`Array`对象](#array对象)
      - [数组调用方法](#数组调用方法)
      - [Array prototype函数（数组原型）](#array-prototype函数数组原型)
      - [字符串与数组转换](#字符串与数组转换)
    - [`proxy`代理对象](#proxy代理对象)
  - [页面宽高](#页面宽高)
    - [`offset、scroll、client`](#offsetscrollclient)
  - [语法运用](#语法运用)
    - [获取随机索引值](#获取随机索引值)
    - [提取非行间样式](#提取非行间样式)
    - [自制滚动条](#自制滚动条)
...
- [ES6](#es6)
    ...
    - [原型](#原型)
      ...
      - [对象原型`__proto__`](#对象原型__proto__)
      - [`constructor`构造函数、构造函数原型和对象三者关系](#constructor构造函数构造函数原型和对象三者关系)
      - [原型链](#原型链)
      - [扩展内置对象](#扩展内置对象)
    - [`Array`的扩展方法](#array的扩展方法)
      - [迭代遍历方法：](#迭代遍历方法)
      - [`forEach()`遍历数组](#foreach遍历数组)
      - [`map()`遍历数组](#map遍历数组)
      - [`filter()`遍历数组](#filter遍历数组)
      - [`new Array().fill()`创建数组并赋值](#new-arrayfill创建数组并赋值)
      - [`copyWith()`拷贝数组元素到指定位置](#copywith拷贝数组元素到指定位置)
    - [`Object`的扩展方法](#object的扩展方法)
      - [`defineProperty()`新增或修改原有属性](#defineproperty新增或修改原有属性)
      - [对象的拓展运算符 （...）](#对象的拓展运算符-)
    - [this](#this-1)
      - [函数的定义、调用与`this`的指向](#函数的定义调用与this的指向)
      - [改变函数内部`this`指向](#改变函数内部this指向)
        - [借用构造函数继承父类型属性、方法-->`call()`](#借用构造函数继承父类型属性方法--call)
        - [改变函数内部的`this`指向](#改变函数内部的this指向)
      - [`call()、bind()、apply()`的区别和使用](#callbindapply的区别和使用)
    - [严格模式](#严格模式)
    - [预解释](#预解释)
    - [JS中内存的分类](#js中内存的分类)
    - [作用域](#作用域)
      - [块作用域（ES6新增）](#块作用域es6新增)
    - [闭包](#闭包)
    - [递归遍历多层`json`数据](#递归遍历多层json数据)
    - [深拷贝与浅拷贝](#深拷贝与浅拷贝)
        - [浅拷贝的实现方式](#浅拷贝的实现方式)
        - [深拷贝的实现方式](#深拷贝的实现方式)
    - [`let`、`var`、`const`](#letvarconst)
      - [`let`、`var`、`const`叙述](#letvarconst叙述)
    - [ES6新增语法](#es6新增语法)
      - [**函数和属性增项写法**](#函数和属性增项写法)
      - [值互换](#值互换)
      - [解构赋值](#解构赋值)
      - [剩余(`rest`)参数](#剩余rest参数)
      - [arguments](#arguments)
      - [剩余参数和 `arguments`对象的区别](#剩余参数和-arguments对象的区别)
      - [剩余参数与结构配合](#剩余参数与结构配合)
      - [扩展运算符`...`](#扩展运算符)
      - [Array的扩展方法](#array的扩展方法-1)
        - [**构造函数方法**](#构造函数方法)
        - [**实例方法**](#实例方法)
    ...
    - [数据结构](#数据结构)
      - [Set数据结构](#set数据结构)
      - [`map`数据结构](#map数据结构)
      - [`WeakMap()`数据结构](#weakmap数据结构)
    - [symbol](#symbol)
      - [`Symbol.for, Symbol.keyFor()` 重新使用同一个Symbol值](#symbolfor-symbolkeyfor-重新使用同一个symbol值)
    - [高阶函数](#高阶函数)
      - [箭头函数](#箭头函数)
    ...
    - [正则表达式](#正则表达式)
    - [Promise对象](#promise对象)
      - [Promise对象的特点](#promise对象的特点)
      - [Promise 优缺点](#promise-优缺点)
      - [Promise创建](#promise创建)
      - [`Promise.prototype.then`方法：链式操作](#promiseprototypethen方法链式操作)
      - [Promise Ajax](#promise-ajax)
      - [`Promise.all`方法，`Promise.race`方法](#promiseall方法promiserace方法)
    - [`async、await`异步函数（阻塞）](#asyncawait异步函数阻塞)
- [JS排序算法](#js排序算法)
  - [冒泡排序](#冒泡排序)
    - [双向冒泡排序（鸡尾酒排序）](#双向冒泡排序鸡尾酒排序)
  - [选择排序](#选择排序)
  - [插入排序](#插入排序)
    - [直接插入排序](#直接插入排序)
    - [折半（二分）插入排序](#折半二分插入排序)
...
- [JQ](#jq)
  - [理念](#理念)
    - [load 和 ready 谁先执行？](#load-和-ready-谁先执行)
  - [选择器](#选择器)
    - [基本选择器](#基本选择器)
    - [层次选择器](#层次选择器)
    - [基本过滤选择器](#基本过滤选择器)
...
- [less](#less)
    - [`less`安装/引入](#less安装引入)
      - [在vue项目中使用less](#在vue项目中使用less)
    - [`less`编译成`wxss`文件](#less编译成wxss文件)
    - [混合`Mixins`](#混合mixins)
    - [嵌套`Nesting`](#嵌套nesting)
    - [@规则嵌套和冒泡](#规则嵌套和冒泡)
    - [运算`Operations`](#运算operations)
    - [`calc()`特例](#calc特例)
    - [转义`Escaping`](#转义escaping)
    - [函数`Functions`](#函数functions)
    - [映射`Maps`](#映射maps)
    - [作用域`Scope`](#作用域scope)
    - [导入`importing`](#导入importing)
- [软件使用](#软件使用)
  - [安装nodejs](#安装nodejs)
  - [Node](#node)
    - [NPM](#npm)
  - [国内镜像（国内加速访问GitHub）](#国内镜像国内加速访问github)
  - [Yarn](#yarn)
  - [NVM](#nvm)
  - [Webpack](#webpack)
  - [Babel](#babel)
...
- [面试题](#面试题)
  - [CSS](#css-1)
    ...
    - [移动端适配1px问题](#移动端适配1px问题)
    - [居中为什么要使用transform(为什么不适用margin:Left/Top)](#居中为什么要使用transform为什么不适用marginlefttop)
  - [JS/ES6](#jses6)
    - [ES6的新特性](#es6的新特性)
    - [promise的使用、怎么用es5实现promise](#promise的使用怎么用es5实现promise)
    - [if有作用域吗](#if有作用域吗)
    - [原型链和作用域链的区别](#原型链和作用域链的区别)
  - [Webpack](#webpack-1)
    - [webpack里面的插件是怎么实现的](#webpack里面的插件是怎么实现的)
    - [dev-server是怎么跑起来](#dev-server是怎么跑起来)
    - [使用过webpack里面哪些plugin和loader](#使用过webpack里面哪些plugin和loader)
    - [webpack整个生命周期，loader和plugin有什么区别](#webpack整个生命周期loader和plugin有什么区别)
    - [webpack打包的整个过程](#webpack打包的整个过程)
    - [一般怎么组织CSS（Webpack）](#一般怎么组织csswebpack)
    - [如何配置把js、css、html单独打包成一个文件](#如何配置把jscsshtml单独打包成一个文件)
    - [webpack和gulp的优缺点](#webpack和gulp的优缺点)
    - [使用webpack构建时有无做一些自定义操作](#使用webpack构建时有无做一些自定义操作)
- [Vue2.0](#vue20)
  - [安装](#安装)
    - [VScode插件](#vscode插件)
      - [快捷生成`console.log()`](#快捷生成consolelog)
    - [vscode问题](#vscode问题)
  - [VUE的认知](#vue的认知)
    - [提高开发效率的发展历程](#提高开发效率的发展历程)
    - [框架和库的区别](#框架和库的区别)
    - [node（后端）中的 MVC 与前端中的 MVVM 之间的区别](#node后端中的-mvc-与前端中的-mvvm-之间的区别)
    - [Vue的生命周期](#vue的生命周期)
    - [vue的两个核心](#vue的两个核心)
    - [方法和函数的区别](#方法和函数的区别)
  - [模板语法](#模板语法)
    - [插值](#插值)
      - [文本：](#文本)
      - [原始HTML `v-html`](#原始html-v-html)
      - [文本形式`v-text`](#文本形式v-text)
      - [不解析直接输出内容`v-pre`](#不解析直接输出内容v-pre)
      - [先解析再渲染`v-cloak`](#先解析再渲染v-cloak)
    - [v-bind:（缩写`：`）](#v-bind缩写)
      - [使用v-bind绑定class样式](#使用v-bind绑定class样式)
      - [使用v-bind绑定内联样式](#使用v-bind绑定内联样式)
    - [v-on:（缩写`@`）](#v-on缩写)
    - [`v-on`事件修饰符](#v-on事件修饰符)
    - [数据双向绑定`v-model`](#数据双向绑定v-model)
    - [循环`v-for和key属性`](#循环v-for和key属性)
    - [vue循环赋值](#vue循环赋值)
    - [方法](#方法-1)
    - [v-if](#v-if)
    - [vue中的this](#vue中的this)
  ...
  - [组件](#组件)
    ...
    - [组件name的作用](#组件name的作用)
    - [**父子组件的通信**](#父子组件的通信)
    - [**自定义事件`$emit与$on`**](#自定义事件emit与on)
    - [跨多级组件(嵌套关系)的通信](#跨多级组件嵌套关系的通信)
      - [非父子组件间的传递`this.$bus.$on`与`this.$bus.$emit`](#非父子组件间的传递thisbuson与thisbusemit)
      - [例：A/C组件属于夸多级的组件嵌套关系，需实现数据通信](#例ac组件属于夸多级的组件嵌套关系需实现数据通信)
      - [**父子组件的访问方式**](#父子组件的访问方式)
        - [**子组件的methods中获取props中的值**](#子组件的methods中获取props中的值)
    - [slot组件的插槽](#slot组件的插槽)
      - [普通插槽（携带的默认值可替换）](#普通插槽携带的默认值可替换)
      - [**具名插槽**（多个插槽）](#具名插槽多个插槽)
      - [**作用域插槽`slot-scope属性`**](#作用域插槽slot-scope属性)
  - [响应式原理](#响应式原理)
    - [动态修改`Vue`项目中的页面标题`title`](#动态修改vue项目中的页面标题title)
    - [vue中响应式值变化处理`this.$set()`](#vue中响应式值变化处理thisset)
      - [对于对象](#对于对象)
      - [对于数组](#对于数组)
      - [对于 数组中嵌套对象](#对于-数组中嵌套对象)
    - [重置vue的data中的数据](#重置vue的data中的数据)
    - [`this.$nextTick()`异步更新队列](#thisnexttick异步更新队列)
    - [无痕刷新](#无痕刷新)
  ...
  - [路由](#路由)
    - [**路由**](#路由-1)
    - [**渲染**](#渲染)
    - [**hash 和 history**](#hash-和-history)
    - [**路由的默认值和修改为history模式**](#路由的默认值和修改为history模式)
    - [**通过代码修改路由**](#通过代码修改路由)
    - [动态路由](#动态路由)
    - [打包文件的解析（路由的懒加载）](#打包文件的解析路由的懒加载)
    - [嵌套路由（子路由）](#嵌套路由子路由)
    - [参数传递](#参数传递)
    - [`$route`与`$router`区别](#route与router区别)
    - [`keep-alive`与`vue-router`](#keep-alive与vue-router)
    - [配置路径别名(文件路径引用问题)](#配置路径别名文件路径引用问题)
    - [账号登录缓存](#账号登录缓存)
    - [导航守卫（可做权限验证）](#导航守卫可做权限验证)
      - [全局前置守卫`router.beforeEach()`](#全局前置守卫routerbeforeeach)
      - [全局解析守卫`router.beforeResolve()`](#全局解析守卫routerbeforeresolve)
      - [全局后置钩子`router.afterEach()`](#全局后置钩子routeraftereach)
      - [路由独享守卫](#路由独享守卫)
      - [组件内的守卫](#组件内的守卫)
      - [**完整的导航解析流程**](#完整的导航解析流程)
  - [Vuex](#vuex)
      - [理念](#理念-1)
      - [使用`vuex`管理状态](#使用vuex管理状态)
    - [State（单一状态树）](#state单一状态树)
    - [Getters](#getters)
    - [Mutation（传递参数、状态更新）](#mutation传递参数状态更新)
      - [mutation的提交风格](#mutation的提交风格)
    - [Vuex的响应式原理](#vuex的响应式原理)
      - [mutation的类型常量](#mutation的类型常量)
      - [mutation同步函数](#mutation同步函数)
    - [Actions](#actions)
    - [moudules（模块）](#moudules模块)
      - [模块的局部状态](#模块的局部状态)
    - [项目结构](#项目结构)
  - [axios](#axios)
      - [安装/使用axios](#安装使用axios)
      - [axios请求方式](#axios请求方式)
      - [全局配置](#全局配置)
      - [创建axios实例](#创建axios实例)
      - [axios的拦截器`interceptors`](#axios的拦截器interceptors)
  - [nginx](#nginx)
  - [问题](#问题-1)
      - [`NavigationDuplicated: Avoided redundant navigation to current location`](#navigationduplicated-avoided-redundant-navigation-to-current-location)
      - [命令行报错：`Component template should contain exactly one root element. If you are using v-if on multiple elements, use v-else-if to chain them instead `](#命令行报错component-template-should-contain-exactly-one-root-element-if-you-are-using-v-if-on-multiple-elements-use-v-else-if-to-chain-them-instead-)
      - [渲染错误问题：`“Error in render: "TypeError: Cannot read property '0' of undefined"”`](#渲染错误问题error-in-render-typeerror-cannot-read-property-0-of-undefined)
  - [案例](#案例-1)
    - [跑马灯效果](#跑马灯效果)
    - [用户登录案例](#用户登录案例)
    - [父子组件通信案例](#父子组件通信案例)
    - [`router_cli@2`多页面内容切换--`TabBar`](#router_cli2多页面内容切换--tabbar)
      - [选项卡切换](#选项卡切换)
- [Echarts](#echarts)
  - [起步](#起步)
      - [注意](#注意)
      - [建议：](#建议)
  - [demo](#demo)
    - [图表大小随窗口变化自适应](#图表大小随窗口变化自适应)
  - [图状](#图状)
      - [柱状图`bar`](#柱状图bar)
      - [折线图`line`](#折线图line)
      - [散点图`scatter`](#散点图scatter)
      - [直角坐标系](#直角坐标系)
      - [饼图`pie`](#饼图pie)
- [小程序](#小程序)
  - [描述](#描述)
    - [vscode开发微信小程序安装的插件](#vscode开发微信小程序安装的插件)
    - [小程序代码构成](#小程序代码构成)
      - [.json配置](#json配置)
        - [小程序配置app.json](#小程序配置appjson)
        - [页面配置page.json](#页面配置pagejson)
        - [微信索引sitemap.json](#微信索引sitemapjson)
      - [.wxml 模板](#wxml-模板)
      - [wxss 样式](#wxss-样式)
      - [.js逻辑交互](#js逻辑交互)
    - [渲染层与逻辑层](#渲染层与逻辑层)
    - [小程序架构->`MVVM`模式](#小程序架构-mvvm模式)
  - [标签](#标签-1)
    - [单选框`radio`](#单选框radio)
    - [`icon`标签](#icon标签)
    - [`view`标签](#view标签)
    ...
    - [block标签](#block标签)
    - [image标签](#image标签)
  - [swiper标签（轮播图）](#swiper标签轮播图)
  - [组件](#组件-1)
    - [tabBar配置（底部导航栏）](#tabbar配置底部导航栏)
    - [navigator导航栏组件](#navigator导航栏组件)
    - [父子组件通信](#父子组件通信)
  - [动态](#动态)
    - [冒泡事件](#冒泡事件)
    - [数据绑定](#数据绑定)
    - [自定义下拉刷新](#自定义下拉刷新)
    - [`input`事件绑定](#input事件绑定)
  - [属性](#属性)
    - [条件渲染`wx:if、wx:elif、wx:else`](#条件渲染wxifwxelifwxelse)
    - [`wx:if`与`hidden`属性的区别](#wxif与hidden属性的区别)
    - [列表渲染`wx:for`](#列表渲染wxfor)
    - [overflow属性](#overflow属性)
    - [box-sizing属性](#box-sizing属性)
  - [样式](#样式)
    - [图片大小](#图片大小)
        - [**图片高度自适应**](#图片高度自适应)
    - [`tap-highlight-color`点击高亮透明](#tap-highlight-color点击高亮透明)
    - [文本行数](#文本行数)
    - [尺寸单位`rpx`](#尺寸单位rpx)
- [node.js](#nodejs)
  - [初识](#初识)
  ...
  - [`stream`流](#stream流)
      - [`pipe`读写流](#pipe读写流)
  - [`http`网络请求](#http网络请求)
- [桌面应用——electron开发](#桌面应用electron开发)
- [窗口命令行](#窗口命令行)
  - [Mac命令](#mac命令)
    - [windows命令下载文件](#windows命令下载文件)
  - [Linux](#linux)
    - [**下载文件**](#下载文件)
    - [移动命令mv](#移动命令mv)
    - [删除文件](#删除文件)
...
- [Git](#git)
    ...
    - [创建仓库](#创建仓库)
    - [文件添加到暂存区](#文件添加到暂存区)
    - [暂存区文件存放到仓库](#暂存区文件存放到仓库)
    - [查看状态](#查看状态)
    - [查看提交记录](#查看提交记录)
    - [版本回退/还原](#版本回退还原)
    - [撤销修改](#撤销修改)
    - [删除仓库文件](#删除仓库文件)
    - [提交代码到GitHub](#提交代码到github)
      - [将本地代码提交远程仓库时新建一个分支](#将本地代码提交远程仓库时新建一个分支)
    - [ssh](#ssh)
    - [删除远程库绑定(GitHub库)](#删除远程库绑定github库)
    - [删除远程库文件](#删除远程库文件)
    - [克隆GitHub项目到本地](#克隆github项目到本地)
    - [分支管理](#分支管理)
      - [合并分支概念](#合并分支概念)
    - [解决合并分支内容的冲突](#解决合并分支内容的冲突)
    - [分支管理策略](#分支管理策略)
    - [Bug分支](#bug分支)
    - [Feature分支](#feature分支)
    - [多人协作](#多人协作)
    - [Rebase](#rebase)
    - [标签管理](#标签管理)
    - [同机关联gitee与github（并行）](#同机关联gitee与github并行)
    - [error:链接Gitee的push/pull出现错误](#error链接gitee的pushpull出现错误)
    - [忽略特殊文件](#忽略特殊文件)
- [Windows:Windows会自动在有图片的目录下生成隐藏的缩略图文件，如果有自定义目录，目录下就会有Desktop.ini文件，因此你需要忽略Windows自动生成的垃圾文件：](#windowswindows会自动在有图片的目录下生成隐藏的缩略图文件如果有自定义目录目录下就会有desktopini文件因此你需要忽略windows自动生成的垃圾文件)
- [Python，忽略Python编译产生的.pyc、.pyo、dist等文件或目录](#python忽略python编译产生的pycpyodist等文件或目录)
- [My configurations:自定义](#my-configurations自定义)
- [排除所有.开头的隐藏文件:](#排除所有开头的隐藏文件)
- [排除所有.class文件:](#排除所有class文件)
- [不排除.gitignore和App.class:](#不排除gitignore和appclass)
    - [配置别名](#配置别名)
    - [搭建Git服务器](#搭建git服务器)
    - [使用SourceTree](#使用sourcetree)
- [记注](#记注)
    - [互联网公司的岗位英文简写](#互联网公司的岗位英文简写)
    - [学习路线](#学习路线)
    - [学习框架方法](#学习框架方法)
- [Emmet(缩写)语法](#emmet缩写语法)
    - [元素](#元素)
    - [子：`>`](#子)
    - [兄弟：`+`](#兄弟)
    - [上升：`^`](#上升)
    ...
    - [属性运算符](#属性运算符)
      ...
      - [自定义属性](#自定义属性)
    - [编号：`$`](#编号)
      - [改变编号的基数和方向：`@`](#改变编号的基数和方向)
        - [在`$`后添加`@-`来改变数字的走向：](#在后添加-来改变数字的走向)
        - [在`$`后添加`@N`改变编号的基数：](#在后添加n改变编号的基数)
        - [同时改变编号的基数和方向：](#同时改变编号的基数和方向)
    - [文本：`{}`](#文本-1)
    - [缩写格式的注意事项](#缩写格式的注意事项)
- [PHP](#php)
    - [让文字不乱码](#让文字不乱码)
    - [把图片路径存入数据库，前台显示图片](#把图片路径存入数据库前台显示图片)
