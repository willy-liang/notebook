...
- **函数防抖**是指频繁触发的情况下，只有足够的空闲时间，才执行代码一次；无空闲时间则不执行。
...
- 有些监听器函数的执行需要多次、大量的完成，而有些繁重的会把浏览器拖死。
  但我们既不能让浏览器被拖垮，又不能删除删除监听器；但我们可以限制函数调用的频度，弱化事件函数运行带来的影响。
- 最常见的使用降频操作的地方：窗口的resize事件，根据用户的按键输入给出自动补全提示。
  - 浏览器窗口的resize事件这种事件会在浏览器窗口大小的每一尺度变化都触发一次，如果监听器体量很大，浏览器很快会被拖垮。相对于让窗口的每一步size的变化都触发一次监听器函数，我们可以现在监听函数的触发的最小间隔必须大于多少毫秒，让它保持着合理的调用频道，确保不毁坏用户体验。
...
function debounce(func, wait=500, immediate=false) {
  ...
  return function () {
    ...
    var later = function () {
      timeout = null;
      if (!immediate) typeof func === 'function' && func.apply(context, args);
    };
...
在编写代码时，如果某个属性不为 null 和 undefined，那么就获取该属性，如果该属性为 null 或 undefined，则取一个默认值：`const name = dogName ? dogName : 'default'; `

可以通过 || 来简化：`const name =  dogName || 'default'; `
