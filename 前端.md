...
- `301 Moved Permanently`请求的网页已永久移动到新的url（存在缓存）
...
- `401 Unauthorized` 表示发送的请求需要通过http认证的认证信息(请求未授权)
...
- `501 Not Implemented` 表示服务器不支持当前请求所需的某个功能
- `502 Bad Gateway` 作为网关或者代理工作的服务器尝试执行请求时，从远程服务器接收到一个无效的响应
- `503 Service Unavailable` 服务器端暂时无法处理请求（可能服务器暂时处于超过载或停机维护）
...
- 都可以改变函数执行时的上下文（改变函数内部的`this`指向）
...
5.query地址栏中有参数信息 eg:http://localhost:8080/#/spoc-eduManage/classManage/addStudent.html?id=274de6fe001f474b89d4ed89a9a7197
```App.vue
<template>
  <div id="app">
