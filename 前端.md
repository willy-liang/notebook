...
let reslut = fruits.entries();
console.log(reslut.next().value[0])  // 0
console.log(reslut.next().value)  // [ 1, 'Orange' ]
```
...
####  复用同一个Symbol值`Symbol.for/Symbol.keyFor()` 
...
- finally()方法表示无论promise实例最终失败还是成功都会执行的方法
