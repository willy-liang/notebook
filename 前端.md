...
- call和bind方法可以接收多个参数，apply方法只能接收2个参数，且第二个参数为数组或伪数组`[arg]`
...
- 生命周期钩子 = 生命周期函数 = 生命周期事件 = callHook(回调钩子)
 ...
 - **创建期间的生命周期函数**：
   ...
   + **created** 实例已经创建完成之后被调用。此时实例已完成数据观测(data observer)、属性和方法的运算、watch/event 事件回调。此时没有$el，但可通过 vm.$nextTick 来访问 Dom
