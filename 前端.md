...
- `opacity:0`：本质上是将元素的透明度将为0，但依然占据空间且可以交互
  - （场景：可以跟transition搭配；重建图层，性能较高）
- `visibility:hidden`: 占据空间，但不可交互
  - （场景：显示不会导致页面结构发生变动，不会撑开；重回操作 比回流操作性能高一些）
- `overflow:hidden`:只隐藏元素溢出的部分，但是占据空间且不可交互
- `display:none`: 元素从文档流中消失彻底隐藏，不占据空间、不交互也不影响布局
  - （场景：显示出原来这里不存在的结构，会回流操作，性能开销较大）
- `z-index:-9999`: 将层级放到底部，这样就会被其他元素被覆盖
- `transform: scale(0,0)`: 平面变换，将元素缩放为0，但依然占据空间且不可交互
...
onclick：点击事件				onfocus：获取聚焦事件    onblur：失去焦点事件		
...
**1、NaN属性：-->用于引用特殊的非数字值**(Not a Number，表示不是一个数字)
...
- slice(start, end)：截取起始位置到结束位置的部分数据(返回数组)
