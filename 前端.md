...
- w3c规范：display属性为block, list-item ,table的元素才会产生BFC，且BFC仍属于文档中的普通流
- 产生BFC的元素
  1. 根元素
  2. float属性不为none；
...
- `Object.freeze('被冻结的对象') `方法可以冻结一个对象，即该对象不可修改/添加属性，该对象的原型也不能被修改。该方法返回被冻结的对象。
...
- 生命周期钩子 = 生命周期函数 = 生命周期事件 = callHook(回调钩子)
 ...
 - **创建期间的生命周期函数**：
   ...
   + beforeCreate：实例刚在内存中被创建出，此时还没初始化 props、data 和 methods 属性
 ...
 - **销毁期间的生命周期函数**：(vue实例一般不会销毁，一般组件会销毁)
   ...
   - destroyed： 实例销毁后调用。调用后，Vue 实例指示的所有变量都会解除绑定，所有的事件监听器会被移除，所有的子实例也会被销毁
