...
> null和undefined
> var、const、let
> 使得通过const定义的对象里面的数据不能修改（通过Object.definePrototype限制属性不可修改、还有冻结对象）
> 判断array是什么数据类型
> 输入url到页面显示所经历的过程
> 
> http缓存，强缓存与协商缓存
> http缓存是在第一次获取资源后，根据返回的信息来告诉如何缓存资源
...
> 
> localStorage存储的是JSON对象，存储之前需要通过JSON.stringify()把对象转化为JSON字符串，后续操作该JSON对象，要将存储的JSON字符串转化为JSON对象再进行操作
> var obj = {"a": 1,"b": 2};
...
> #### 重绘和回流(重排)  
>
> > 1. **重绘**：不影响页面布局的操作，如DOM改动、CSS改动、改变某个元素的背景色、文字颜色、边框颜色
> > 2. **重排**：重新渲染排布页面布局
> >    - 浏览器页面初始化、添加/删除可见的DOM元素、元素位置、尺寸、内容改变、窗口尺寸改变
> > 3. **注意：回流必将引起重绘，重绘不一定会引起回流**
> >
> > **减少重绘和重排的方法**：不在布局信息改变时做 DOM 查询、使用class类定义要修改的属性。对于重排的元素使其脱离文档流再改变
> >
> > - 避免使用 table 布局。
...
> })
> 
> 注意：mutations必须是同步函数。因为异步方法不知道什么时候状态会改变，就无法追踪了
>   export default {
...
> - 相对定位：相对于自身进行定位，元素仍然占据原来的位置
> - 绝对定位：相对于离自己最近的定义了相对定位或绝对定位的父元素位置定位，若没有，则相对于body左上角定位，会脱标，不占位置
> - 固定定位：以body左上角作定位，元素不占位
> - 静态定位：元素的标准流显示方式，不可移动元素
...
> - **闭包是指有权访问另一个函数作用域中变量的函数**(常见的就是在函数内部创建函数调用该函数的)
> - 闭包用途
