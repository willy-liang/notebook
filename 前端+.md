...
> - box-sizing：content-box指定盒子模型为标准盒模型，border-box 为怪异盒模型，inherit为从父元素继承box-sizing属性的值
> 
> 标准盒模型：content 为 width 和 height
> 怪异盒模型：content 为 width 和 height、border及padding
> ```
...
> - 当对象查找一个属性时，如果没有在自身找到，则会逐级向上查找自身的原型，直到找到 Object.prototype 原型为 null 时查找停止。这种逐级向上查找原型链称为原型链
...
>    - 具体渲染过程包括：构建DOM树、构建CSS规则树、构建render树、进行页面布局、遍历render树绘制页面元素
>
> #### 重绘和回流(重排)  
>
